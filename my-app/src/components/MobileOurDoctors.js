...
const mobileOurDoctors = DOCTORS.map(therapist => {
    return (
        <Card className='mobile-clinic-doctor-avatar-card'>
            <DoctorImage src={therapist.image} alt={therapist.alt} />
    ...
    return (
        <OurTherapistStyles className='container'>
            ...
            <div className='row m-4 d-md-none'>
                <Carousel show={2} slide={1} responsive={true} swiping={true} rightArrow={<FontAwesomeIcon icon={faAngleRight} size='2x' className='fontawesome-carousel-icon'/>} leftArrow={<FontAwesomeIcon icon={faAngleLeft} size='2x' className='fontawesome-carousel-icon'/>}>
                  {mobileOurDoctors}
                </Carousel>
            </div>
            <div className='row m-2'>
                <Link to='/DoctorsPage' style={{textDecoration: 'none'}}>
